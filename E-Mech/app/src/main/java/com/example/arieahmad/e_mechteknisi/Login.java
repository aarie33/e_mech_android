                                    ...
                                    try {
                                        ...
                                        if (jsonObject.getBoolean("error") == false) {
                                            SharedPrefManager.getInstance(getApplicationContext())
                                                    .teknisiLogin(username, jsonObject.getString("nama"),
                                                            jsonObject.getString("status"));
                                            if (jsonObject.getString("status").equals("0")) {
                                                startActivity(new Intent(getApplicationContext(), HomeTeknisiBaru.class));
                                            }else{
                                                startActivity(new Intent(getApplicationContext(), HomeTeknisi.class));
                                            }
                                            finish();
                                        }
